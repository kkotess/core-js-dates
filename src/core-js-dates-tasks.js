...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstDate = Date.parse(dateStart);
  const secondDate = Date.parse(dateEnd);

  const counterDays = (secondDate - firstDate) / 86400000;
  return counterDays + 1;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${a}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let weekendCount = 0;
  for (let day = 1; day <= dateInMonth; day += 1) {
